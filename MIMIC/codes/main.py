...
if __name__ == '__main__':
    ...
    res_folder="./MIMIC_RESULTS/"

    data_module = DataModuleForMIMIC(args)
    lightning_model = LightningForMIMIC(args)

    logger = pl.loggers.CSVLogger(res_folder, name=args.run_name, flush_logs_every_n_steps=30)

    ckpt_callbacks = ModelCheckpoint(monitor='Val/mrr', save_weights_only=True, mode='max')
    #early_stop_callback = EarlyStopping(monitor="Val/mrr", min_delta=0.00, patience=20, verbose=True, mode="max")

    trainer = pl.Trainer(**args.trainer,
                         deterministic=True, logger=logger, default_root_dir=res_folder,
